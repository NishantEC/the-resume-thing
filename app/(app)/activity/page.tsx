  ...
  return (
    <div className="mx-auto max-w-[740px] animate-[screenIn_.4s_ease] px-[44px] pb-20 pt-12">
      <div className="mb-[30px] flex flex-col gap-[10px]">
        <span className="font-mono text-[12px] text-muted-foreground">step 2 — ingest</span>
      ...
      {total === 0 ? (
        <div className="flex flex-col items-center gap-[14px] rounded-[16px] border border-dashed border-border bg-card p-[46px] text-center">
          <span className="text-[15px] text-muted-foreground">Nothing ingested yet.</span>
          <Link
            href="/sources"
        ...
        <div className="flex flex-col gap-[22px]">
          <div className="grid grid-cols-3 gap-[10px]">
            {stats.map((t) => (
              <div
                key={t.label}
                className="flex flex-col gap-[5px] rounded-[12px] border border-border bg-card px-[14px] pb-[13px] pt-[14px] shadow-[0_1px_2px_rgba(0,0,0,0.04)]"
              >
                <span className="text-[26px] font-semibold leading-none tracking-[-0.02em] text-foreground">
                  {t.n}
                </span>
                <span className="text-[12px] leading-[1.2] text-muted-foreground">{t.label}</span>
              </div>
            ))}
          </div>
          ...
          <div className="overflow-hidden rounded-[16px] border border-border bg-card shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
            ...
            {recent.map((a) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-[12px] border-b border-border px-[18px] py-[11px] no-underline hover:bg-accent"
              >
                <span
                ...
                <span className="w-[74px] flex-none text-right font-mono text-[10.5px] text-muted-foreground">
                  {a.type}
                </span>
              </a>
            ))}
          ...
          <div className="flex items-center justify-between gap-[16px] rounded-[14px] border border-border bg-card px-[20px] py-[16px]">
